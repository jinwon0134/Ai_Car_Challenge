...
def startTimer():
    ...
    if connectstate == 1 :
        try :
            timer = threading.Timer(0.1, startTimer)
            timer.start()
            sensor = SensorFun(10)        
...
def SensorFun(command) :
    try :        
        ...
        sensordata1.CNT = sensordata1.CNT+0.1
            
        if not sensor.FLAG:
            if sensordata1.IR[3] > 20 and sensordata1.IR[2] > 15:
                steering(-63)
            elif sensordata1.IR[1] > 20 and sensordata1.IR[2] > 15:
                steering(63)
            else:
                if sensordata1.IR[3] > 10:
                    steering(-50)
                elif sensordata1.IR[1] > 10:
                    steering(50)
